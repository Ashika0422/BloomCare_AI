...
const FREQ_CFG = {
  'Daily':           { color: '#5A8A72', bg: '#EBF4EF', border: '#A8D5B8', icon: '🔄' },
...
const s = {
  ...
  notifBanner: (granted) => ({
    ...
    background: granted ? '#EBF4EF' : '#FDF3E7',
    border: `1px solid ${granted ? '#A8D5B8' : '#F5C97B'}`,
  ...
  notifBtn: (granted) => ({
    ...
    background: granted ? 'transparent' : '#D4924A',
    color: granted ? '#9A6B1A' : '#fff',
    border: `1.5px solid ${granted ? '#F5C97B' : '#D4924A'}`,
  ...
  progressFill: (pct) => ({
    ...
    background: 'linear-gradient(90deg, #5A8A72, #3D6B54)',
  ...
  todayMed: (taken) => ({
    ...
    background: taken ? '#EBF4EF' : 'var(--blush)',
    border: `1.5px solid ${taken ? '#A8D5B8' : 'var(--border)'}`,
    transition: 'all 0.2s ease',
  }),
  todayCheck: (taken) => ({
    width: 28, height: 28, borderRadius: '50%', flexShrink: 0,
    background: taken ? '#5A8A72' : 'transparent',
    border: `2px solid ${taken ? '#5A8A72' : 'var(--border)'}`,
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    cursor: 'pointer', transition: 'all 0.2s ease', fontSize: 13,
    color: '#fff',
  }),
  todayMedName: (taken) => ({
    flex: 1, fontSize: 15, fontWeight: 600,
    color: taken ? '#2D7A4F' : 'var(--slate)',
    textDecoration: taken ? 'line-through' : 'none',
    transition: 'all 0.2s',
  }),
  todayMedTime: { fontSize: 12, color: 'var(--slate-light)', fontWeight: 500 },
  freqPill: (cfg) => ({
  ...
  emptyToday: {
    ...
    color: 'var(--slate-light)', fontSize: 14,
  },
  emptyIcon: { fontSize: 40, marginBottom: 10, display: 'block' },
  ...
  medMeta: { fontSize: 12, color: 'var(--slate-light)', lineHeight: 1.5 },
  ...
  formCard: {
    background: 'var(--white)', borderRadius: 'var(--radius-xl)',
    border: '1px solid var(--border)', padding: '28px 30px',
    boxShadow: 'var(--shadow-sm)', position: 'sticky', top: 82,
  ...
  freqBtn: (selected, cfg) => ({
    padding: '8px 6px', borderRadius: 10, textAlign: 'center',
    border: `1.5px solid ${selected ? cfg.color : 'var(--border)'}`,
    background: selected ? cfg.bg : 'var(--blush)',
    cursor: 'pointer', transition: 'all 0.18s ease', fontFamily: 'var(--font-body)',
    boxShadow: selected ? `0 0 0 2px ${cfg.color}22` : 'none',
  }),
  freqBtnIcon: { fontSize: 16, display: 'block', marginBottom: 2 },
  freqBtnLabel: (selected, color) => ({
  ...
  successMsg: {
    background: '#EBF4EF', border: '1px solid #A8D5B8', borderRadius: 10,
    padding: '10px 14px', fontSize: 13, color: '#2D7A4F',
...
export default function MedicineTracker() {
  ...
  const [takenMap,   setTakenMap]   = useState({});   // id → bool, stored in localStorage
  ...
  const [form, setForm] = useState({
    medicine_name: '', time_to_take: '',
    frequency: 'Daily', days_of_week: [], notes: '',
  });

  /* ── Load taken state from localStorage ──────────────────── */
  useEffect(() => {
    const today = new Date().toISOString().split('T')[0];
    const key   = `bb_taken_${today}`;
    const saved = localStorage.getItem(key);
    if (saved) {
      try { setTakenMap(JSON.parse(saved)); } catch {}
    }
  }, []);
  ...
  const handleRequestNotif = async () => {
    ...
    if (granted) {
      setSuccess('Notifications enabled! You will be reminded to take your medicines.');
  ...
  const handleAdd = async () => {
    ...
    if (form.frequency === 'Weekly' && !form.days_of_week.length)
      errs.days_of_week = 'Pick at least one day';
    ...
    setLoading(false);
    if (res.success) {
      setForm({ medicine_name: '', time_to_take: '', frequency: 'Daily', days_of_week: [], notes: '' });
      setSuccess('Medicine added successfully!');
      setTimeout(() => setSuccess(''), 3000);
      fetchData();
    } else {
      setFormErr(res.errors?.join(' · ') || res.error || 'Failed to add medicine.');
    }
  };

  /* ── Delete ───────────────────────────────────────────────── */
  const handleDelete = async (id) => {
    await authFetch(`/medicines/${id}`, { method: 'DELETE' });
    fetchData();
  };

  /* ── Toggle active ────────────────────────────────────────── */
  const handleToggleActive = async (id) => {
    await authFetch(`/medicines/${id}/toggle`, { method: 'PATCH' });
    fetchData();
  };
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.pageHeader}>
        ...
        <h1 style={s.pageTitle}>Your Medications 💊</h1>
      ...
      <div style={s.notifBanner(notifPerm === 'granted')}>
        <div style={s.notifLeft}>
          <span style={s.notifIcon}>{notifPerm === 'granted' ? '🔔' : '🔕'}</span>
      ...
      <div style={s.layout}>
          ...
          <div style={s.card(100)}>
            ...
            {todayMeds.length === 0 ? (
              <div style={s.emptyToday}>
                ...
                <span style={{ color: 'var(--slate-mid)', fontSize: 13 }}>
                  Add a medicine using the form on the right.
                </span>
              </div>
            ) : (
              todayMeds.map(med => {
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 3 }}>
                        <span style={s.todayMedTime}>⏰ {med.time_to_take}</span>
                        <span style={s.freqPill(cfg)}>{cfg.icon} {med.frequency}</span>
                        {med.notes && (
                    ...
                    {taken && (
                      <span style={{
                        fontSize: 11, fontWeight: 700, color: '#2D7A4F',
                        background: '#EBF4EF', padding: '2px 9px',
              ...
              allMeds.map(med => {
                ...
                return (
                  <div key={med.id} style={{
                    ...
                    <div style={s.medIconWrap(cfg)}>{cfg.icon}</div>
                    <div style={s.medInfo}>
                    ...
                    <div style={s.medActions}>
                      <button
                        style={s.iconBtn(med.is_active ? '#9A6B1A' : '#2D7A4F', med.is_active ? '#FDF3E7' : '#EBF4EF')}
                        title={med.is_active ? 'Pause' : 'Resume'}
        ...
        <div style={s.formCard}>
          ...
          {formErr  && <div style={s.errMsg}>⚠️ {formErr}</div>}
          ...
          <div style={s.fieldWrap}>
            ...
            <input
              ...
              placeholder="e.g. Folic Acid 400mcg"
              ...
              {FREQUENCIES.map(f => {
                ...
                return (
                  <div key={f} style={s.freqBtn(sel, cfg)} onClick={() => setField('frequency', f)}>
                    <span style={s.freqBtnIcon}>{cfg.icon}</span>
            ...
            <div style={s.fieldWrap}>
              ...
              <div style={s.daysRow}>
                {DAYS.map(d => (
                  <button
                    key={d}
                    style={s.dayBtn(form.days_of_week.includes(d))}
                    onClick={() => toggleDay(d)}
                  >
                    {d}
                  </button>
          ...
          <button
            ...
            onMouseLeave={e => { e.currentTarget.style.transform = 'none'; }}
          >
            {loading ? <><div style={s.spinner} /> Adding…</> : '💊 Add Medicine'}
          </button>

          <p style={{
            marginTop: 14, fontSize: 11, color: 'var(--slate-light)',
            lineHeight: 1.6, textAlign: 'center',
          }}>
            Reminders fire at the scheduled time via browser notifications.
            The app must be open in a tab for reminders to work.
          </p>
        </div>
      </div>
    </div>
