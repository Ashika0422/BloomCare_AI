...
import {
  Flower2, Send, Loader2, X, Trash2, Zap, Sparkles,
...
function renderMarkdown(text) {
  ...
  lines.forEach((line, idx) => {
    ...
    if (line.startsWith('### ')) { flushList(); out.push(<p key={idx} style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 600, color: 'var(--slate)', margin: '10px 0 5px' }}>{line.slice(4)}</p>); return; }
    if (line.startsWith('## ')) { flushList(); out.push(<p key={idx} style={{ fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 600, color: 'var(--slate)', margin: '12px 0 5px' }}>{line.slice(3)}</p>); return; }
    const numMatch = line.match(/^(\d+)\.\s+(.+)/);
    if (numMatch) {
      flushList();
      out.push(<div key={idx} style={{ display: 'flex', gap: 10, marginBottom: 6, fontSize: 14, lineHeight: 1.65 }}><span style={{ minWidth: 20, height: 20, borderRadius: '50%', background: 'rgba(232,102,122,0.15)', color: 'var(--primary-dark)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0, marginTop: 2 }}>{numMatch[1]}</span><span>{inlineFormat(numMatch[2])}</span></div>);
      return;
    }
    if (line.startsWith('- ') || line.startsWith('* ')) { listItems.push(line.slice(2)); return; }
    if (line.trim() === '---') { flushList(); out.push(<hr key={idx} style={{ border: 'none', borderTop: '1px solid var(--border)', margin: '8px 0' }} />); return; }
    if (line.startsWith('> ')) { flushList(); out.push(<div key={idx} style={{ borderLeft: '3px solid var(--primary)', paddingLeft: 12, margin: '5px 0', color: 'var(--slate-mid)', fontSize: 13, fontStyle: 'italic' }}>{inlineFormat(line.slice(2))}</div>); return; }
    flushList();
...
function TypingDots() {
  return (
    <div style={{ display: 'flex', gap: 4, alignItems: 'center', height: 20 }}>
      {[0, 1, 2].map(i => (
        <div key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--primary-light)', animation: `typingBounce 1.2s ease-in-out ${i * 0.18}s infinite` }} />
      ))}
...
const PROVIDER_CFG = {
  groq: {
    label: 'Groq · Llama 3.1', Icon: Zap,
    color: '#F97316', bg: '#FFF7ED', border: '#FED7AA',
    desc: 'Fast, free, no credit card',
  },
  gemini: {
    label: 'Google Gemini', Icon: Sparkles,
...
const s = {
  ...
  providerBtn: (active, cfg) => ({ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 99, border: `1.5px solid ${active ? cfg.color : 'var(--border)'}`, background: active ? cfg.bg : 'transparent', color: active ? cfg.color : 'var(--slate-mid)', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.18s ease', fontFamily: 'var(--font-body)' }),
  onlineDot: { width: 7, height: 7, borderRadius: '50%', background: '#5A8A72', animation: 'onlinePulse 2.5s ease-in-out infinite' },
  clearBtn: { padding: '5px 12px', borderRadius: 99, border: '1.5px solid var(--border)', background: 'transparent', color: 'var(--slate-mid)', fontSize: 12, fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'var(--font-body)', display: 'flex', alignItems: 'center', gap: 5 },
  setupBanner: { margin: '12px 20px', padding: '14px 18px', background: '#FFF7ED', border: '1.5px solid #FED7AA', borderRadius: 14, animation: 'fadeIn 0.3s ease' },
  ...
  setupCard: (color, bg, border) => ({ padding: '12px 14px', borderRadius: 10, background: bg, border: `1.5px solid ${border}` }),
  setupCardTitle: (color) => ({ fontSize: 13, fontWeight: 700, color, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 6 }),
  setupStep: { fontSize: 12, color: 'var(--slate-mid)', lineHeight: 1.6 },
  setupLink: (color) => ({ color, fontWeight: 600, textDecoration: 'none', borderBottom: `1px solid ${color}44` }),
  ...
  suggTitle: { fontSize: 11, fontWeight: 700, color: 'var(--slate-mid)', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 10, alignSelf: 'flex-start', width: '100%', maxWidth: 560 },
  suggChip: { padding: '10px 12px', borderRadius: 11, background: 'var(--white)', border: '1.5px solid var(--border)', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s ease', fontFamily: 'var(--font-body)', display: 'flex', alignItems: 'center', gap: 8 },
...
export default function ChatAssistant() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/chat/providers', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then(d => { if (d.success) { setProviderStatus({ ...d, checked: true }); if (d.gemini && !d.groq) setProvider('gemini'); } })
      .catch(() => setProviderStatus(p => ({ ...p, checked: true })));
  }, [token]);

  useEffect(() => {
    fetch('http://localhost:5000/chat/suggestions', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
    ...
    <div style={s.page}>
      ...
      <div style={s.header}>
        <div style={s.headerAvatar}>
          <Flower2 size={20} color="#fff" strokeWidth={1.8} />
        </div>
      ...
      {showSetup && (
        <div style={s.setupBanner}>
          <div style={s.setupTitle}>Set up your free AI provider to start chatting</div>
          <div style={s.setupGrid}>
            <div style={s.setupCard('#F97316', '#FFF7ED', '#FED7AA')}>
              <div style={s.setupCardTitle('#C2410C')}>
                <Zap size={13} strokeWidth={2} /> Groq (Recommended)
              </div>
              <div style={s.setupStep}>
                1. Go to{' '}<a href="https://console.groq.com" target="_blank" rel="noreferrer" style={s.setupLink('#F97316')}>console.groq.com</a><br />
                2. Sign up free (email only, no card)<br />
                3. Click <strong>API Keys → Create API Key</strong><br />
                4. Add to your <code style={{ fontSize: 11 }}>.env</code>:<br />
                <code style={{ fontSize: 11, background: 'rgba(0,0,0,0.06)', padding: '2px 5px', borderRadius: 4 }}>GROQ_API_KEY=gsk_...</code>
              </div>
            </div>
            <div style={s.setupCard('#4285F4', '#EFF6FF', '#BFDBFE')}>
              <div style={s.setupCardTitle('#1D4ED8')}>
                <Sparkles size={13} strokeWidth={2} /> Google Gemini (Fallback)
              </div>
              <div style={s.setupStep}>
                1. Go to{' '}<a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" style={s.setupLink('#4285F4')}>aistudio.google.com</a><br />
                2. Sign in with your Google account<br />
                3. Click <strong>Get API Key → Create</strong><br />
                4. Add to your <code style={{ fontSize: 11 }}>.env</code>:<br />
                <code style={{ fontSize: 11, background: 'rgba(0,0,0,0.06)', padding: '2px 5px', borderRadius: 4 }}>GEMINI_API_KEY=AIza...</code>
              </div>
            </div>
          </div>
      ...
      {!hasMessages ? (
        <div style={s.welcome}>
          <div style={s.welcomeOrb}>
            <Flower2 size={38} color="var(--primary)" strokeWidth={1.4} />
          </div>
          <h2 style={s.welcomeTitle}>Hello, {user?.full_name?.split(' ')[0]}!</h2>
            ...
            <div key={i} style={s.msgRow(msg.role === 'user')}>
              {msg.role === 'assistant' && (
                <div style={s.botAvatar}>
                  <Flower2 size={14} color="#fff" strokeWidth={1.8} />
                </div>
              )}
                ...
                <div style={s.bubbleTime(msg.role === 'user')}>
                  ...
                  {msg.role === 'assistant' && (
                    <span style={{ marginLeft: 6, fontSize: 9, opacity: 0.6 }}>
                      {provider === 'groq' ? 'Llama 3.1' : 'Gemini'}
                    </span>
                  )}
                </div>
              </div>
          ...
          {(loading || streaming) && (
            <div style={s.msgRow(false)}>
              ...
              <div style={s.bubble(false)}>
                {loading
                  ? <TypingDots />
                  : <div style={{ color: 'var(--slate)' }}>{renderMarkdown(streamText)}<span style={s.cursor} /></div>
                }
              </div>
            </div>
          )}
      ...
      {error && (
        <div style={s.errBar}>
          ...
          <button onClick={() => setError('')} style={{ background: 'none', border: 'none', color: 'var(--primary-dark)', fontWeight: 700, cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center' }}>
      ...
      <div style={s.inputBar}>
        ...
        <button
          ...
          {loading || streaming
            ...
            : <Send size={18} color={input.trim() ? '#fff' : 'var(--slate-light)'} strokeWidth={2} />
          }
        </button>
      </div>
    </div>
