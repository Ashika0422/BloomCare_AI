...
const VITALS = [
  ...
  },
  {
    ...
    icon: '🩸', color: '#D4924A', lightColor: '#FDF3E7',
  ...
  },
  {
    ...
    icon: '🌡️', color: '#5A8A72', lightColor: '#EBF4EF',
  ...
  },
  {
    ...
    icon: '📈', color: '#546A7B', lightColor: '#EDF2F5',
...
const s = {
  ...
  statusIcon: { fontSize: 20, marginBottom: 6 },
  statusVal: {
    fontFamily: 'var(--font-display)', fontSize: 22,
    fontWeight: 600, color: 'var(--slate)', lineHeight: 1, marginBottom: 2,
  },
  statusLabel: { fontSize: 11, color: 'var(--slate-light)', marginBottom: 6 },
  statusBadge: (status) => ({
    display: 'inline-block', fontSize: 10, fontWeight: 700,
    padding: '2px 7px', borderRadius: 99,
    background: STATUS_CFG[status]?.bg || 'var(--blush)',
    color: STATUS_CFG[status]?.color || 'var(--slate-mid)',
  ...
  inputUnit: {
    ...
    fontWeight: 600, color: 'var(--slate-light)',
  ...
  /* Result card */
  resultCard: (cfg) => ({
    borderRadius: 'var(--radius-xl)', border: `1.5px solid ${cfg.border}`,
    overflow: 'hidden', boxShadow: 'var(--shadow-md)',
    animation: 'bounceIn 0.5s cubic-bezier(.36,.07,.19,.97) both',
  }),
  resultHeader: (cfg) => ({
    background: cfg.grad, padding: '28px 32px 24px',
  ...
  resultLabelLg: (color) => ({
    ...
    fontWeight: 600, color, lineHeight: 1.1,
  }),
  confidenceWrap: {
  ...
  messageBanner: (cfg) => ({
    ...
    fontSize: 14, color: 'var(--slate)', lineHeight: 1.7,
  }),
  actionBanner: (cfg) => ({
  ...
  tab: (active, color) => ({
    ...
    background: active ? color : 'var(--blush)',
  ...
  histTd: {
    padding: '11px 12px', fontSize: 13, color: 'var(--slate)',
  ...
  return (
    <div style={s.rangeBar}>
      <div style={{
        ...
        width: `${pct}%`,
        background: isNormal ? '#5A8A72' : color,
...
export default function RiskAnalysis() {
  ...
  /* ── Fetch trends & history ─────────────────────────────── */
  const fetchData = useCallback(async () => {
    const [t, h] = await Promise.all([
      authFetch('/vitals/trends?limit=15'),
      authFetch('/vitals/history?limit=10'),
    ]);
  ...
  const validate = () => {
    ...
    VITALS.forEach(v => {
      ...
      if (isNaN(n) || n < v.min || n > v.max)
        errs[v.key] = `${v.min}–${v.max} ${v.unit}`;
    });
    setErrors(errs);
    return Object.keys(errs).length === 0;
  };

  /* ── Get live status of a field ─────────────────────────── */
  ...
  const handleAnalyse = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setApiErr(res.errors?.join(' · ') || res.error || 'Prediction failed.');
      }
    } catch {
      setApiErr('Cannot reach the server. Make sure Flask is running.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const combinedData = trends?.timestamps?.map((t, i) => {
    const row = { time: t };
    VITALS.forEach(v => {
      row[v.label] = trends.trend_data[v.dbKey]?.[i];
    });
    return row;
  }) || [];

  const singleData = trends?.timestamps?.map((t, i) => ({
    time:  t,
    value: trends.trend_data[activeTab.replace('_', '') === 'combined' ? 'systolic_bp' : activeTab]?.[i],
  })) || [];
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.pageHeader}>
        ...
        <h1 style={s.pageTitle}>Risk Analysis 🩺</h1>
        <p style={s.pageSub}>
          Log your vitals, view trends, and get an AI-powered health risk assessment.
        </p>
      </div>

      {/* ── Latest vitals status strip ──────────────────── */}
          ...
          {VITALS.map(v => {
            ...
            return (
              <div key={v.key} style={s.statusCard(v, status)}>
                ...
                <div style={{ ...s.statusIcon, marginTop: 8 }}>{v.icon}</div>
            ...
            return (
              <div key={v.key} style={s.vitalField}>
                <div style={s.vitalHeader}>
                  ...
                  <span style={s.vitalNormal}>
                    Normal: {v.normalMin}–{v.normalMax} {v.unit}
                  </span>
                </div>
                <div style={s.inputRow}>
                  <input
                    ...
                    onChange={e => {
                      setForm(p => ({ ...p, [v.key]: e.target.value }));
                      if (errors[v.key]) setErrors(p => ({ ...p, [v.key]: null }));
                    }}
                  />
                  <span style={s.inputUnit}>{v.unit}</span>
                </div>
                <RangeFill
                  value={form[v.key]} min={v.min} max={v.max}
        ...
        <div style={s.rightCol}>
          ...
          {result ? (() => {
              ...
              <div style={s.resultCard(cfg)}>
                ...
                <div style={s.resultHeader(cfg)}>
                  <div style={s.resultTop}>
                    <span style={s.resultEmoji}>{cfg.icon}</span>
                    <div>
                ...
                <div style={s.resultBody}>
                  ...
                  <div style={s.actionBanner(cfg)}>
                    <span>📋</span><span>{result.action}</span>
                  </div>
                  ...
                  <div style={s.xaiSection}>
                    ...
                    <p style={{ fontSize: 12, color: 'var(--slate-light)', marginBottom: 14, lineHeight: 1.5 }}>
                      How much each vital sign influenced this prediction, based on model feature importance.
                    </p>
                    {result.feature_impact?.map((f, i) => {
                      const vitalCfg = VITAL_MAP[f.key];
                      return (
                        <div key={f.key} style={s.xaiRow(f.status)}>
                          <span style={s.xaiIcon}>{vitalCfg?.icon || '📊'}</span>
                          <div style={s.xaiInfo}>
                            <div style={s.xaiLabel}>{f.label}</div>
                            <div style={s.xaiVal(f.status)}>
                              {f.value} {f.unit}
                              {f.status !== 'normal' && ` · ${f.status.toUpperCase()}`}
                            </div>
                          </div>
            ...
            <div style={s.empty}>
              ...
              <p style={s.emptySub}>
                Enter your vitals on the left and click<br />
                <strong>Analyse My Risk</strong> to see your assessment here.
              </p>
            </div>
          )}
          ...
          <div style={s.card}>
            ...
            <div style={s.tabRow}>
              <button
                style={s.tab(activeTab === 'combined', '#546A7B')}
              ...
              {VITALS.map(v => (
                <button
                  key={v.dbKey}
                  style={s.tab(activeTab === v.dbKey, v.color)}
                  onClick={() => setActiveTab(v.dbKey)}
                >
                  {v.icon} {v.label}
                </button>
              ))}
            ...
            {combinedData.length > 0 ? (
              activeTab === 'combined' ? (
                <ResponsiveContainer width="100%" height={280}>
                  <LineChart data={combinedData} margin={{ left: -10, right: 10 }}>
                    ...
                    {VITALS.map(v => (
                      <Line
                        key={v.label} type="monotone"
                        dataKey={v.label} stroke={v.color}
                ...
                (() => {
                  ...
                  const chartData = trends?.timestamps?.map((t, i) => ({
                    time:  t,
                    value: trends.trend_data[activeTab]?.[i],
                  })) || [];
                  return (
                    <div>
                      {vCfg && (
                        <div style={{
                          ...
                          <span style={{ fontSize: 13, color: 'var(--slate-mid)' }}>
                            <span style={{ color: '#5A8A72', fontWeight: 600 }}>■</span> Normal: {vCfg.normalMin}–{vCfg.normalMax} {vCfg.unit}
                          </span>
                        </div>
                      )}
              ...
              <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--slate-light)', fontSize: 14 }}>
                No trend data yet. Log your first vitals above to see charts.
              </div>
            )}
          </div>
            ...
            <div style={s.card}>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={s.histTable}>
                  <thead>
                    <tr>
                      {['Date', 'Sys BP', 'Dia BP', 'Glucose', 'Temp', 'HR', 'Risk', 'Confidence'].map(h => (
