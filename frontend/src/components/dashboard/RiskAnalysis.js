...
const s = {
  ...
  input: (focused, status) => ({ width: '100%', padding: '11px 52px 11px 14px', borderRadius: 'var(--radius-md)', border: `1.5px solid ${status === 'high' ? '#E8CCFF' : status === 'low' ? '#F5C97B' : focused ? 'var(--primary)' : 'var(--border)'}`, background: status === 'high' ? '#F5E6FF' : status === 'low' ? '#FDF3E7' : focused ? 'var(--blush)' : '#fdfaf7', fontSize: 15, color: 'var(--slate)', outline: 'none', fontFamily: 'var(--font-body)', transition: 'all 0.2s ease', boxShadow: focused ? '0 0 0 3px rgba(237,18,164,0.15)' : 'none' }),
