...
import {
  Users, UserPlus, Stethoscope, BookOpen, Pill, Target,
  Download, Eye, Trash2, Shield, ShieldCheck, ShieldAlert, ShieldX,
  Leaf, Flower2, Loader2, AlertTriangle, FileText, Lock,
} from 'lucide-react';
...
const TRIM_CFG = {
  1: { label: '1st', Icon: Leaf,    color: '#5A8A72' },
  2: { label: '2nd', Icon: Flower2, color: '#D4924A' },
  3: { label: '3rd', Icon: Flower2, color: '#8A00F3' },
};
...
const s = {
  ...
  promoteBtn: { padding: '12px 28px', borderRadius: 'var(--radius-md)', background: 'linear-gradient(135deg, #2C3E50, #546A7B)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 600, fontFamily: 'var(--font-body)', boxShadow: '0 4px 18px rgba(44,62,80,0.3)', transition: 'all 0.2s', display: 'inline-flex', alignItems: 'center', gap: 8 },
  statsRow: { display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 14, marginBottom: 28, animation: 'fadeUp 0.5s ease 0.05s both' },
  statCard: (color) => ({ background: 'var(--white)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', padding: '16px 18px', boxShadow: 'var(--shadow-sm)', position: 'relative', overflow: 'hidden' }),
  statBar: (color) => ({ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: color, borderRadius: '10px 10px 0 0' }),
  statIcon: { marginBottom: 6, marginTop: 4 },
  statVal: { fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 600, color: 'var(--slate)', lineHeight: 1, marginBottom: 3 },
  statLabel: { fontSize: 10, color: 'var(--slate-light)', fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase' },
  chartsRow: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 18, marginBottom: 28, animation: 'fadeUp 0.5s ease 0.1s both' },
  card: (delay = 0) => ({ background: 'var(--white)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', padding: '22px 24px', boxShadow: 'var(--shadow-sm)', animation: `fadeUp 0.5s ease ${delay}ms both` }),
  cardTitle: { fontFamily: 'var(--font-display)', fontSize: 17, fontWeight: 600, color: 'var(--slate)', marginBottom: 3 },
  ...
  td: { padding: '12px 12px', fontSize: 13, color: 'var(--slate)', borderBottom: '1px solid var(--border)', verticalAlign: 'middle' },
  userNameCell: { display: 'flex', alignItems: 'center', gap: 10 },
  userAvatar: (color) => ({ width: 32, height: 32, borderRadius: '50%', flexShrink: 0, background: `linear-gradient(135deg, ${color}, #6E01F4)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#fff', fontWeight: 700 }),
  userName: { fontSize: 13, fontWeight: 600, color: 'var(--slate)' },
  userEmail: { fontSize: 11, color: 'var(--slate-light)', marginTop: 1 },
  trimPill: (cfg) => ({ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 99, color: cfg.color, background: cfg.color + '18', border: `1px solid ${cfg.color}44` }),
  riskPill: (cfg) => ({ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 99, background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.border}` }),
  actionBtn: (color, bg) => ({ padding: '5px 10px', borderRadius: 8, background: bg, color, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 600, transition: 'all 0.15s', fontFamily: 'var(--font-body)', whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: 5 }),
  pagination: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 18, paddingTop: 14, borderTop: '1px solid var(--border)' },
  pageInfo: { fontSize: 13, color: 'var(--slate-mid)' },
  pageBtn: (active) => ({ padding: '5px 12px', borderRadius: 8, border: `1.5px solid ${active ? 'var(--slate)' : 'var(--border)'}`, background: active ? 'var(--slate)' : 'transparent', color: active ? '#fff' : 'var(--slate-mid)', fontSize: 12, fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'var(--font-body)', margin: '0 2px' }),
  overlay: { position: 'fixed', inset: 0, background: 'rgba(44,62,80,0.5)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, animation: 'fadeIn 0.2s ease' },
  modal: { background: 'var(--white)', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: 680, maxHeight: '88vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(44,62,80,0.25)', animation: 'bounceIn 0.35s cubic-bezier(.36,.07,.19,.97) both' },
  modalHeader: { padding: '22px 28px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
  modalTitle: { fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 600, color: 'var(--slate)' },
  closeBtn: { width: 32, height: 32, borderRadius: '50%', background: 'var(--blush)', border: '1px solid var(--border)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  modalBody: { padding: '20px 28px 28px' },
  infoGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 22 },
  infoItem: { background: 'var(--blush)', borderRadius: 10, padding: '10px 14px', border: '1px solid var(--border)' },
...
export default function AdminPanel() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    const params = new URLSearchParams({ page, per_page: 15, ...(search ? { search } : {}), ...(trimFilter ? { trimester: trimFilter } : {}), ...(riskFilter ? { risk: riskFilter } : {}) });
    const d = await authFetch(`/admin/users?${params}`);
    if (d.success) { setUsers(d.users); setTotalUsers(d.total); setPages(d.pages); }
  ...
  const handlePromote = async () => {
    ...
    setPromoting(false);
    if (res.success) { updateUser(res.user); setPromoteMsg(''); }
    else setPromoteMsg(res.error || 'Failed.');
  };
  ...
  const handleDeleteUser = async (uid) => {
    ...
    setSelectedUser(null); setUserDetail(null); fetchUsers();
  };

  const downloadPDF = async (uid, username) => {
    setDownloading(uid);
    try {
      const res = await fetch(`http://localhost:5000/reports/user/${uid}`, { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) throw new Error('PDF generation failed');
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = `BloomCare_report_${username}.pdf`; a.click();
      URL.revokeObjectURL(url);
    } catch { alert('PDF download failed. Make sure reportlab is installed.'); }
  ...
  const downloadMyPDF = async () => {
    setDownloading('me');
    try {
      const res = await fetch('http://localhost:5000/reports/my', { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) throw new Error();
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = `BloomCare_report_${user.username}.pdf`; a.click();
      URL.revokeObjectURL(url);
    } catch { alert('PDF download failed. Make sure reportlab is installed: pip install reportlab'); }
    finally { setDownloading(null); }
  };
  ...
  const activityData = stats?.daily_activity?.map(d => ({ date: new Date(d.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), users: d.users })) || [];

  if (!isAdmin) {
    return (
      <div style={s.page}>
        <div style={s.denied}>
          <Lock size={64} color="var(--border)" strokeWidth={1.2} style={{ display: 'block', margin: '0 auto' }} />
          ...
          {promoteMsg && <p style={{ color: 'var(--primary-dark)', fontSize: 14, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center' }}><AlertTriangle size={14} strokeWidth={2} />{promoteMsg}</p>}
          <button style={s.promoteBtn} onClick={handlePromote} disabled={promoting}
          ...
          <div style={{ marginTop: 32, padding: '20px 24px', background: 'var(--white)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', display: 'inline-block' }}>
            <p style={{ fontSize: 14, color: 'var(--slate-mid)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
              <FileText size={14} strokeWidth={1.8} /> You can still download your own health report:
            </p>
            <button style={s.pdfBtn} onClick={downloadMyPDF} disabled={downloading === 'me' || !reportAvail}>
              {downloading === 'me'
                ? <><Loader2 size={14} style={{ animation: 'spin 0.7s linear infinite' }} /> Generating…</>
                : <><Download size={14} strokeWidth={1.8} /> Download My PDF Report</>
              }
            </button>
          </div>
        </div>
    ...
    <div style={s.page}>
      <div style={s.pageHeader}>
        <div style={s.headerRow}>
          <div>
            <div style={s.eyebrow}><Shield size={11} strokeWidth={2} /> Admin Panel</div>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginTop: 8 }}>
            <div style={s.adminBadge}>
              <Shield size={13} strokeWidth={1.8} /> {user?.full_name?.split(' ')[0]} · Admin
            </div>
            <button style={s.pdfBtn} onClick={downloadMyPDF} disabled={downloading === 'me' || !reportAvail}
              ...
              {downloading === 'me'
                ? <><Loader2 size={14} style={{ animation: 'spin 0.7s linear infinite' }} /> Generating…</>
                : <><Download size={14} strokeWidth={1.8} /> My Report</>
              }
            </button>
          </div>
        </div>
        ...
        <div style={s.statsRow}>
          {STAT_CARDS.map((st, i) => (
            <div key={i} style={s.statCard(st.color)}>
              ...
              <div style={{ ...s.statIcon, marginTop: 8 }}>
                <st.Icon size={20} color={st.color} strokeWidth={1.6} />
              </div>
              <div style={s.statVal}>{stats[st.key]}</div>
      ...
      <div style={s.chartsRow}>
        <div style={s.card(100)}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie data={riskChartData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={3} dataKey="value">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={trimChartData} margin={{ left: -20 }}>
              ...
              <Bar dataKey="value" name="Users" radius={[4, 4, 0, 0]}>
                {trimChartData.map((entry, i) => <Cell key={i} fill={entry.fill} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div style={s.card(200)}>
          ...
          ) : <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--slate-light)', fontSize: 13 }}>No activity data yet.</div>}
        </div>
      </div>

      <div style={s.tableCard}>
        <div style={s.tableTopRow}>
          <h2 style={s.tableTitle}>All Users <span style={{ fontSize: 13, color: 'var(--slate-light)', fontWeight: 400, marginLeft: 10 }}>{totalUsers} total</span></h2>
          <input placeholder="Search name, email, username…" value={search}
            style={s.searchInput(focusSearch)}
            onFocus={() => setFocusSearch(true)} onBlur={() => setFocusSearch(false)}
            onChange={e => { setSearch(e.target.value); setPage(1); }} />
          <select style={s.filterSelect} value={trimFilter} onChange={e => { setTrimFilter(e.target.value); setPage(1); }}>
            <option value="">All Trimesters</option>
            <option value="1">1st Trimester</option>
            <option value="2">2nd Trimester</option>
            <option value="3">3rd Trimester</option>
          </select>
          <select style={s.filterSelect} value={riskFilter} onChange={e => { setRiskFilter(e.target.value); setPage(1); }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={s.table}>
            <thead>
              <tr>{['User', 'Age', 'Trimester', 'Last Risk', 'Confidence', 'Predictions', 'Joined', 'Actions'].map(h => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {loadingUsers ? (
                  ...
                  <tr key={u.id}
                    ...
                    <td style={s.td}>
                      <div style={s.userNameCell}>
                        <div style={s.userAvatar(avatarColor)}>{u.full_name?.[0]?.toUpperCase()}</div>
                        <div>
                    ...
                    <td style={s.td}>
                      <span style={s.trimPill(trimCfg)}>
                        <trimCfg.Icon size={9} strokeWidth={2} /> {trimCfg.label}
                      </span>
                    </td>
                    <td style={s.td}>
                      {riskCfg
                        ? <span style={s.riskPill(riskCfg)}><riskCfg.Icon size={10} strokeWidth={2} />{riskCfg.label}</span>
                        : <span style={{ color: 'var(--slate-light)', fontSize: 12 }}>—</span>}
                    </td>
                    <td style={s.td}>{u.last_conf != null ? `${u.last_conf}%` : '—'}</td>
                    ...
                    <td style={s.td}>
                      <div style={{ display: 'flex', gap: 5 }}>
                        ...
                        <button style={s.actionBtn('#8A00F3', '#F5E6FF')} disabled={downloading === u.id || !reportAvail}
        ...
        {pages > 1 && (
          <div style={s.pagination}>
            <span style={s.pageInfo}>Showing {users.length} of {totalUsers} users</span>
            <div>
              <button style={s.pageBtn(false)} disabled={page === 1} onClick={() => setPage(p => p - 1)}>← Prev</button>
              {Array.from({ length: Math.min(pages, 7) }, (_, i) => i + 1).map(p => (
                <button key={p} style={s.pageBtn(p === page)} onClick={() => setPage(p)}>{p}</button>
              ))}
              <button style={s.pageBtn(false)} disabled={page === pages} onClick={() => setPage(p => p + 1)}>Next →</button>
            </div>
          </div>
        )}
      </div>
          ...
          <div style={s.modal}>
            <div style={s.modalHeader}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button style={s.pdfBtn} disabled={downloading === selectedUser.id || !reportAvail}
                  onClick={() => downloadPDF(selectedUser.id, selectedUser.username)}>
                  {downloading === selectedUser.id
                    ? <><Loader2 size={14} style={{ animation: 'spin 0.7s linear infinite' }} /> Generating…</>
                    : <><Download size={14} strokeWidth={1.8} /> Download PDF</>
                  }
                </button>
                <button style={s.closeBtn} onClick={() => { setSelectedUser(null); setUserDetail(null); }}>
                  <span style={{ fontSize: 16, color: 'var(--slate-mid)' }}>✕</span>
                </button>
              </div>
            </div>
            <div style={s.modalBody}>
              <div style={s.infoGrid}>
                {[
                  ...
                  ['Age', `${selectedUser.age} years`], ['Trimester', `Trimester ${selectedUser.trimester}`],
                  ['Due Date', selectedUser.due_date || 'Not set'], ['Joined', new Date(selectedUser.created_at).toLocaleDateString()],
                  ['Predictions', selectedUser.pred_count], ['Admin', selectedUser.is_admin ? 'Yes' : 'No'],
                ].map(([label, val]) => (
                  ...
                  {userDetail.logs.slice(0, 5).map(log => {
                    ...
                    return (
                      <div key={log.id} style={s.logRow}>
                        <span style={{ minWidth: 110 }}>{new Date(log.logged_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                        <span>SBP: {log.systolic_bp}</span><span>DBP: {log.diastolic_bp}</span>
                        <span>Gluc: {log.blood_glucose}</span><span>HR: {log.heart_rate}</span>
                        {rc && <span style={s.riskPill(rc)}><rc.Icon size={10} strokeWidth={2} />{rc.label}</span>}
              ...
              <div style={{ padding: '14px 16px', background: 'var(--primary-tint)', borderRadius: 10, border: '1px solid var(--primary-light)' }}>
                <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--primary-dark)', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <AlertTriangle size={13} strokeWidth={2} /> Danger Zone
                </p>
                <button style={{ ...s.actionBtn('var(--primary-dark)', 'var(--white)'), border: '1.5px solid var(--primary-light)', padding: '8px 16px', display: 'inline-flex', alignItems: 'center', gap: 6 }}
                  onClick={() => handleDeleteUser(selectedUser.id)}>
                  <Trash2 size={13} strokeWidth={1.8} /> Delete User & All Data
                </button>
              </div>
            </div>
          </div>
